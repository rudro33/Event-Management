...
const Hero = forwardRef((props, ref) => {
    ...
    return (
      <section className="min-h-screen flex flex-col justify-center items-center bg-[#0B0A0F] text-white px-6 py-12">
        ...
        <div className="bg-[#151320] w-full max-w-md rounded-2xl shadow-[0_0_30px_rgba(147,51,234,0.6)] p-6 sm:p-8">
          ...
          <button className="w-full bg-purple-600 py-3 rounded-lg font-semibold hover:bg-purple-700 transition">
            Confirm Purchase
          </button>

          <button
            onClick={() => setShowTicket(false)}
            className="w-full mt-4 border border-gray-500 py-3 rounded-lg text-gray-300 hover:bg-gray-800 transition"
          >
            ← Back to Home
          </button>
        </div>
  ...
  if (showSchedule) {
    return (
      <section className="min-h-screen flex flex-col justify-center items-center bg-[#0B0A0F] text-white pt-24 pb-10 px-6">
        <h2 className="text-3xl md:text-4xl font-bold mb-6 text-purple-400 text-center">
          Event Schedule
        </h2>
        ...
        <div className="w-full max-w-3xl bg-[#151320] rounded-xl shadow-lg p-4 sm:p-6">
          {schedules.map((item) => (
            <div
              key={item.id}
              className="flex flex-col sm:flex-row sm:justify-between sm:items-center border-b border-gray-700 py-3 text-center sm:text-left"
            >
  ...
  return (
    <section
      ref={ref}
      className="min-h-screen flex flex-col justify-center pt-24 pb-10 relative overflow-hidden bg-[#0B0A0F] text-white"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
        <div className="text-center md:text-left">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold mt-4 leading-tight">
        ...
        <div className="relative flex justify-center md:justify-end items-center mt-10 md:mt-0">
          ...
          <div className="absolute bottom-0 left-0 w-32 h-32 sm:w-44 sm:h-44 md:w-56 md:h-56 rounded-[30px] sm:rounded-[40px] overflow-hidden shadow-[0_0_25px_4px_rgba(236,72,153,0.6)] rotate-[8deg] translate-x-6 sm:translate-x-10 translate-y-6 sm:translate-y-10 border-4 border-[#0B0A0F]">
